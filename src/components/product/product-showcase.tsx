...
const ProductShowcase: React.FC<ShowcaseProps> = ({
  ...
  const renderLoading = (
    <div className="grid grid-cols-4 gap-6">
      {[1, 2, 3, 4].map((val) => (
        <div key={val}>
          <Skeleton className="w-[50%] h-16 rounded-md"></Skeleton>
          <Skeleton className="w-[75%] h-6 rounded-md mt-4"></Skeleton>
          <Skeleton className="w-[75%] h-6 rounded-md mt-2"></Skeleton>
        </div>
      ))}
    </div>
  );
